...
| **EP-09** `websocket`            | ✅ 34 tests — registry/eligibility/bridge, and since **EP-13.2 a real `/ws` server**: token verified BEFORE the upgrade (a bad one is a 401 with a body, not a silently closed socket), policy resolved fail-closed, heartbeat, `/ws/stats`. ⬜ deployment (no Dockerfile/k8s, and the gateway has no `/ws` route yet), 09.4 resume/polling — the replay window needs Redis (EP-07.4), so `resume` is refused BY NAME rather than acknowledged and ignored.                                                                                                                                                                                                                                                                                                                                                                                       |
...
**Suggested next tasks:** **finish EP-13.2's deployment** — the `/ws` server exists and is tested,
but nothing routes to it: it has no Dockerfile or k8s manifest, the gateway has no `/ws` route (and
proxying an Upgrade is not the same as proxying a request), and `apps/studio/proxy.conf.json`
forwards only `/auth` and `/api`, so `npm start` in Studio still cannot reach it. Until that lands,
the last hop of the walking skeleton is green in tests and absent in the cluster · `EP-14` HSM
(unblocks EP-15 ingest, which the Studio panel is already built for) · a real MAM state-counts
endpoint for the dashboard (it pages with a 1000-asset cap today) · finish `EP-20.2` per-file rows
after EP-17.8.
