...
| **EP-12** observability          | ✅ 12.4 alerts + golden signals on **every deployed service**; **12.2 metrics store + dashboards** — Prometheus/Grafana in [`infra/k8s/observability`](infra/k8s/observability/), optional per [ADR-0003](docs/adr/0003-observability-stack.md). ⬜ 12.1 logs (Loki/Alloy), 12.3 tracing (blocked on 04.7 spans) |

**424 tests across 14 projects, all green** (counted from `nx run-many -t test`, not carried
...
**Suggested next tasks:** `EP-12.1` (logs — Loki + Alloy, the other half of the overlay; 12.3 tracing
is blocked on `EP-04.7` span plumbing, so a collector would receive nothing) ·
