...
| **EP-07** `@atlas/data`          | ✅ 23 tests + `@atlas/data-pg` on **real Postgres** (21) — outbox persists message **headers**, and a `seen` table now carries the outbox's other half: a consumer's dedup mark commits in the SAME transaction as its writes, so a crash mid-handler cannot suppress the redelivery. ⬜ 07.4–07.6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **EP-09** `websocket`            | ✅ 37 tests — registry/eligibility/bridge, and since **EP-13.2 a real `/ws` server**: token verified BEFORE the upgrade (a bad one is a 401 with a body, not a silently closed socket), policy resolved fail-closed, heartbeat, `/ws/stats`. **Deployed** — k8s manifest, dev NodePort 30081, connection caps (node-wide + per user, since the gateway no longer bounds this path), and a smoke test that drives the whole path. It is **not** behind the gateway: `fetch` cannot upgrade a protocol, so `/ws` is an ingress path rule (one origin to the browser) rather than a gateway route — which also keeps long-lived sockets out of a stateless proxy tier. ⬜ 09.4 resume/polling — the replay window needs Redis (EP-07.4), so `resume` is refused BY NAME rather than acknowledged and ignored.                                                                                                         |
| **EP-10** `iam`                  | ✅ 55 tests, incl. `/metrics` + auth signals (#205) and failed-attempt lockout (#240). Entirely IN-MEMORY — no persistence, hence no outbox, hence **it emits nothing**. ⬜ 10.4 CRUD, 10.6 event emission (blocked on a data layer; see the `permissions.changed` note below)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **EP-13** walking skeleton       | ✅ 12 tests incl. **13.3 one trace across gateway → service → broker → consumer** + **13.4 smoke suite green** (15/15, run against kind **and now in CI** — see below). **13.2 COMPLETE and OBSERVED** — the last hop, outbox → broker → bridge → socket, is asserted by a smoke test that holds a real socket open while a write goes through the gateway, and it passes against a live cluster.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| **EP-11** Studio shell (Angular) | ✅ 11.1 skeleton, **11.2 real sign-in against IAM**, 11.3 the workbench, **11.4 WebSocket client** (queued subscriptions, backoff reconnect; **the `/ws` endpoint it talks to now exists** — EP-13.2 — but is not yet routed through the gateway), **11.6 i18n/RTL** (en/ar + design tokens; the token set is now COMPLETE and the RTL mirroring reaches the side-bar divider and the panel names), 11.7 `can()` rendering — 112 tests. **11.5 generated API clients** — every contract Studio has a client for is in `SPECS`, RIM included (`npm run api:check`).                                                                                                                                                                                                                                                                                                                                                 |
| **EP-20** Studio panels          | ✅ **20.1 Media panel** — recent/search/tag filters against real MAM. **20.2 asset editor** — real Basic-info GET/PATCH, per-field-group rendering, dirty state, Files rendition readiness; per-file rows await EP-17.8 (#153). **20.3 Ingest panel** built but `available: false` — RIM (EP-15) doesn't exist, so its API 404s; route+visibility wait for the service, and upload is rendered disabled because EP-15.1 has no endpoint. **20.4 Search panel**, **20.6 Dashboard** (state counts from MAM's `GET /assets/counts` aggregate; a reader whose grant is narrowed is refused it and falls back to the capped page walk, labelled approximate), **20.9 live updates** wired client-side (panels subscribe via the queued WS client; the EP-13.2 server exists, and end-to-end now waits only on routing `/ws` through the gateway). Browse tree blocked on #260 (categories are a field, not an entity). |
| **EP-17** `mam` (Phase 1)        | ✅ 238 tests — asset core, lifecycle, metadata gate, outbox events, Postgres, deployed, **17.2 extensible metadata**, **17.3 free-form tags**, **17.4 search**, field-group scoping (#225), **state-counts aggregate** (unscoped readers only — it cannot apply the per-asset check). ⬜ 17.7, 17.8 (need services that do not exist yet)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
> be removed without the build saying so. Plus **15 smoke tests** against a deployed cluster, and **CI now runs them** (`.github/workflows/smoke.yml`):
...
**Suggested next tasks:** `EP-14` HSM (unblocks EP-15 ingest, which the Studio panel is already built for) · finish `EP-20.2` per-file rows after
EP-17.8.
...
> **⚠️ `permissions.changed` has three consumers and NO producer.** The gateway reasons about it,
> Studio's `if-can` directive re-renders on it, and the WebSocket service drops live subscriptions
> on it — all built, all tested, all dead in a real deployment, because IAM has no outbox to emit
> it from. So **live revocation does not work end to end yet**: a revoked grant reaches an open
> socket only when the connection drops, or when MAM/websocket's 30-second policy cache expires.
> Do not read the consumer code as evidence the feature works; EP-10.6 is the missing half and it
> is blocked on EP-10.4 plus persistence for IAM.
>
...
> need it — the EP-19 audit sink, EP-15 ingest, EP-16 transcode — do not exist. So rule 4 is a
