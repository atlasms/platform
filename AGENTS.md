...
| **EP-09** `websocket`            | ✅ 16 tests — registry/eligibility/bridge as a LIBRARY only: **no server entrypoint exists** (no `main.ts`, nothing serves `/ws`, no k8s deployment). ⬜ 09.4 reconnect/polling (client-side, needs Studio). The Studio client (EP-11.4) is ready; the endpoint it talks to is EP-13.2's scope.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
**613 tests across 14 test projects, all green** (counted from `nx run-many -t test`, not carried
...
**Suggested next tasks:** `EP-13.2` (the WebSocket server endpoint — the Studio client and all the
panel subscriptions are ready and waiting for it) · `EP-14` HSM (unblocks EP-15 ingest, which the
Studio panel is already built for) · a real MAM state-counts endpoint for the dashboard (it pages
with a 1000-asset cap today) · finish `EP-20.2` per-file rows after EP-17.8.
