...
| **EP-09** `websocket`            | ✅ 34 tests — registry/eligibility/bridge, and since **EP-13.2 a real `/ws` server**: token verified BEFORE the upgrade (a bad one is a 401 with a body, not a silently closed socket), policy resolved fail-closed, heartbeat, `/ws/stats`. **Deployed** — k8s manifest, dev NodePort 30081, and a smoke test that drives the whole path. It is **not** behind the gateway: `fetch` cannot upgrade a protocol, so `/ws` is an ingress path rule (one origin to the browser) rather than a gateway route — which also keeps long-lived sockets out of a stateless proxy tier. ⬜ 09.4 resume/polling — the replay window needs Redis (EP-07.4), so `resume` is refused BY NAME rather than acknowledged and ignored.                                                                                                                              |
| **EP-10** `iam`                  | ✅ 55 tests, incl. `/metrics` + auth signals (#205) and failed-attempt lockout (#240). ⬜ 10.4 CRUD, 10.6 event emission                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **EP-13** walking skeleton       | ✅ 12 tests incl. **13.3 one trace across gateway → service → broker → consumer** + **13.4 smoke suite green** (13/13). **13.2** the last hop — outbox → broker → bridge → socket → Studio — has its server (see EP-09); wiring it into the deployment is what remains.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
**631 tests across 14 test projects, all green** (counted from `nx run-many -t test`, not carried
...
**Suggested next tasks:** **run the EP-13.2 smoke test against a real cluster** — the manifests,
the NodePort and the end-to-end check are written, but nothing has yet executed `npm run smoke`
against a live kind cluster, so "a write becomes a live update" is asserted and not yet observed.
Everything before it is proven; that one check is the last unverified claim in the walking skeleton
· `EP-14` HSM (unblocks EP-15 ingest, which the Studio panel is already built for) · a real MAM
state-counts endpoint for the dashboard (it pages with a 1000-asset cap today) · a **per-node
connection cap** for the WebSocket service (websocket.md §11) — routing `/ws` around the gateway
means the gateway's rate limiting no longer bounds it · finish `EP-20.2` per-file rows after
EP-17.8.
