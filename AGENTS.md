...
| **EP-12** observability          | ✅ 12.4 alerts + golden signals everywhere; **12.1 logs + 12.2 metrics** — Prometheus/Loki/Alloy/Grafana in [`infra/k8s/observability`](infra/k8s/observability/), optional per [ADR-0003](docs/adr/0003-observability-stack.md). ⬜ 12.3 tracing (blocked on 04.7 spans), #245 per-service access logs |
...
**Suggested next tasks:** `EP-04.7` (OTLP span plumbing — it is what unblocks `EP-12.3`, since a
tracing collector currently has nothing to receive) ·
