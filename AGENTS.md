...
| **EP-11** Studio shell (Angular) | ✅ 11.1 skeleton, **11.2 real sign-in against IAM**, 11.3 the workbench, 11.7 `can()` rendering — 74 tests. **11.5 generated API clients** (`npm run api:check`). ⬜ 11.4 ws, 11.6 i18n/RTL                                                                                                                          |
| **EP-20** Studio panels          | ✅ **20.1 Media panel** — recent/search/tag filters against real MAM. **20.2 asset editor in progress** — real Basic-info GET/PATCH, per-field-group rendering, dirty state, and Files rendition readiness; per-file rows await EP-17.8 (#153). Browse tree blocked on #260 (categories are a field, not an entity). |
...
**578 tests across 14 test projects, all green** (counted from `nx run-many -t test`, not carried
...
**Suggested next tasks:** finish `EP-20.2` per-file rows after EP-17.8 · `EP-11.4` (WebSocket client
and live-update plumbing) · `EP-11.6` (i18n/RTL + the full design-token system).
