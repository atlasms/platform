...
import {
  loadConfig, AppError, NotFound, ValidationError, toProblem,
  runWithContext, correlationId, HealthRegistry,
  generateTestKey, verifyJwt, requirePermission, Unauthorized, Forbidden,
  createLogger,
} from '../src/index.ts';

test('loadConfig coerces types and applies defaults', () => {
  const cfg = loadConfig(
    { port: { env: 'PORT', type: 'number', default: 3000 }, debug: { env: 'DEBUG', type: 'boolean', default: false }, name: { env: 'NAME', type: 'string', required: true } },
...
test('loadConfig fails fast with all problems', () => {
  try {
    loadConfig({ port: { env: 'PORT', type: 'number', required: true }, tier: { env: 'TIER', type: 'string', enum: ['a', 'b'] } }, { PORT: 'nope', TIER: 'z' });
    assert.fail('should throw');
  } catch (e) {
    assert.ok(e instanceof ValidationError);
    assert.equal((e as ValidationError).status, 422);
    assert.deepEqual((e as ValidationError).details, ['PORT must be a number', 'TIER must be one of a, b']);
  }
});

test('error taxonomy maps to a consistent problem', () => {
  const p = toProblem(new NotFound('asset X'), 'corr-1');
  assert.deepEqual(p, { code: 'NOT_FOUND', status: 404, message: 'asset X', details: undefined, correlationId: 'corr-1' });
...
test('JWT verifies against a JWKS and enforces permissions', async () => {
  ...
  const token = await key.sign({ sub: 'user-42', permissions: ['asset:approve'], channelId: 'ch12' }, { issuer: 'iam', audience: 'atlas' });
