...
interface Registered { name: string; check: Check; critical: boolean; }

export interface ReadinessReport { status: 'ready' | 'not_ready'; checks: { name: string; ok: boolean; critical: boolean }[]; }
...
export class HealthRegistry {
  ...
  liveness(): { status: 'ok' } { return { status: 'ok' }; }
