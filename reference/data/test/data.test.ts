...
import { openDb, withTransaction, migrate, jsonRepo, jsonTableMigration, SqliteOutboxStore, outboxMigration } from '../src/index.ts';
import { InMemoryBroker, OutboxRelay, type OutboxRecord } from '../../messaging/src/index.ts';

interface AssetRow { id: string; state: string; }
const rec = (id: string, type: string): OutboxRecord => ({ id, message: { id, subject: `atlas.ch12.${type}`, body: { type } } });
...
test('a bad migration fails without half-applying', () => {
  const db = openDb();
  assert.throws(() => migrate(db, [{ id: 'x', up: 'CREATE TABLE oops (' }]), /migration "x" failed/);
...
test('transactional outbox ROLLS BACK both writes on failure (atomicity)', () => {
  ...
  assert.throws(() => withTransaction(s.db, () => {
...
test('SqliteOutboxStore drains through the messaging OutboxRelay', async () => {
  ...
  broker.subscribe('atlas.ch12.>', (m) => { captured.push(m.body); });
