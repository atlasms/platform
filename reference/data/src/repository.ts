...
export function jsonRepo<T extends { id: string }>(db: Db, table: string): JsonRepo<T> {
  return {
    put: (e) => { db.prepare(`INSERT INTO ${table}(id, data) VALUES(?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data`).run(e.id, JSON.stringify(e)); },
    get: (id) => { const r = db.prepare(`SELECT data FROM ${table} WHERE id = ?`).get(id) as { data: string } | undefined; return r ? JSON.parse(r.data) as T : undefined; },
    all: () => (db.prepare(`SELECT data FROM ${table}`).all() as { data: string }[]).map((r) => JSON.parse(r.data) as T),
