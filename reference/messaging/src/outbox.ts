...
export interface OutboxRecord { id: string; message: Message; sentAt?: number; }
...
export class InMemoryOutboxStore implements OutboxStore {
  ...
  async add(record: OutboxRecord): Promise<void> { this.records.push({ ...record }); }
  async listUnsent(limit: number): Promise<OutboxRecord[]> { return this.records.filter((r) => !r.sentAt).slice(0, limit); }
  async markSent(id: string, at: number): Promise<void> { const r = this.records.find((r) => r.id === id); if (r) r.sentAt = at; }
  all(): OutboxRecord[] { return this.records; }
}

export class OutboxRelay {
  constructor(private store: OutboxStore, private broker: Broker, private now: () => number = Date.now) {}
