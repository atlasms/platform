...
export class InMemorySeenStore implements SeenStore {
  ...
  async seen(id: string): Promise<boolean> { return this.ids.has(id); }
  async remember(id: string): Promise<void> { this.ids.add(id); }
}

/** Returns a handler that skips messages whose id was already processed. */
export function idempotent(handler: Handler, store: SeenStore, keyOf: (m: Message) => string = (m) => m.id): Handler {
