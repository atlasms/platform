...
import {
  ulid, isUlid, EVENT_TYPES, isEventType, validatePayload,
  buildEnvelope, follow, validateMessage, subjectFor, type Envelope,
} from '../src/index.ts';
...
test('buildEnvelope + validateMessage round-trip for a real event', () => {
  const msg = buildEnvelope({
    ...
    payload: { assetId: ulid(), approver: 'user-1', approvedAt: new Date().toISOString(), expiresAt: new Date().toISOString() },
  });
...
test('validateMessage rejects a payload missing a required field', () => {
  const msg = buildEnvelope({ type: 'asset.approved', channelId: 'ch12', payload: { approver: 'user-1' } }); // no assetId
  const res = validateMessage(msg as Envelope);
...
test('validatePayload validates a payload directly', () => {
  assert.equal(validatePayload('asset.rejected', { assetId: ulid(), reason: 'blurry' }).valid, true);
...
test('follow threads correlation + causation', () => {
  const root = buildEnvelope({ type: 'ingest.accepted', channelId: 'ch12', payload: { assetId: ulid() } as any });
  const next = follow(root as Envelope, { type: 'asset.created', channelId: 'ch12', payload: { assetId: ulid() } as any });
