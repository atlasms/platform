...
export function loadConfig<S extends ConfigSpec>(
  ...
  // Built incrementally as a loose record, then asserted to Config<S> once on return —
  // the per-field types are enforced by the spec, not inferrable during accumulation.
  ...
  for (const [key, f] of Object.entries(spec)) {
    ...
    if (raw === undefined || raw === '') {
      ...
      if (f.required) {
        problems.push(`${f.env} is required`);
        continue;
      }
      out[key] = undefined;
      continue;
    ...
    if (f.enum && !f.enum.includes(raw)) {
      problems.push(`${f.env} must be one of ${f.enum.join(', ')}`);
      continue;
    }
    if (f.type === 'number') {
      const n = Number(raw);
      if (Number.isNaN(n)) {
        problems.push(`${f.env} must be a number`);
        continue;
      }
      out[key] = n;
    } else if (f.type === 'boolean') {
      if (!/^(true|false|1|0)$/i.test(raw)) {
        problems.push(`${f.env} must be a boolean`);
        continue;
      }
